...
func (c *ConnInfo) RemoteIP() string {
	return c.remoteIP
}
...
func (p *EndPoint) addVirtualConn(connID, remoteID uint32, remoteIP string, c chan *link.Session) {
	...
	session.State = &ConnInfo{connID, remoteID, remoteIP}
	select {
	case c <- session:
	case <-p.closeChan:
	default:
...
func (p *EndPoint) processCmd(buf []byte) {
	...
	case acceptCmd:
		...
		p.free(buf)
		p.addVirtualConn(connID, remoteID, "0.0.0.0", p.acceptChan)
	...
	case connectCmd:
		connID, remoteID, remoteIP := p.decodeConnectCmd(buf)
		p.free(buf)
		p.addVirtualConn(connID, remoteID, remoteIP, p.connectChan)
