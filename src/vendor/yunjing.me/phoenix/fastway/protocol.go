...
const (
	...
	connectCmdRemoteIP = connectCmdRemoteID + cmdIDSize
)

func (p *protocol) decodeConnectCmd(msg []byte) (connID, remoteID uint32, remoteIP string) {
	connID = binary.LittleEndian.Uint32(msg[connectCmdConnID:])
	remoteID = binary.LittleEndian.Uint32(msg[connectCmdRemoteID:])

	raw := make([]byte, len(msg)-connectCmdRemoteIP)
	copy(raw, msg[connectCmdRemoteIP:])
	remoteIP = string(raw)

	return
}

func (p *protocol) encodeConnectCmd(connID, remoteID uint32, remoteIP string) []byte {
	buffer := p.allocCmd(connectCmd, connectCmdSize+len(remoteIP))
	...
	copy(buffer[connectCmdRemoteIP:], []byte(remoteIP))
	return buffer
}
