...
func (g *Gateway) ServeClients(lsn net.Listener, cfg GatewayCfg) {
	g.servers[0] = link.NewServer(
		...
		link.ProtocolFunc(func(rw io.ReadWriter) (link.Codec, error) {
			id := atomic.AddUint32(&g.physicalConnID, 1)
			return g.newCodec(id, rw.(net.Conn), cfg.BufferSize), nil
		}),
		cfg.SendChanSize,
...
func (g *Gateway) acceptVirtualConn(pair [2]*link.Session, session *link.Session, maxConn int) bool {
	...
	for i := 0; i < 2; i++ {
		...
		if pair[i] == session {
			...
			addr := session.Codec().(*codec).conn.RemoteAddr().(*net.TCPAddr)
			g.send(pair[i], g.encodeConnectCmd(connID, remoteID, addr.IP.String()))
		}
	}
	return true
}
